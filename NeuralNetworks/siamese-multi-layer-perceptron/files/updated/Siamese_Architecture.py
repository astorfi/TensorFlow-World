...
def get_batch_MLP(start_idx, end_ind, inputs, labels):
    """Get each batch of data which is necessary for the training and testing phase..
        Args:
          start_idx: The start index of the batch in the whole sample size.
          end_idx: The ending index of the batch in the whole sample size.
...
def neural_network(X, dropout_param):
    ...
    #
    # MLP_INPUT = CNN_output
    MLP_INPUT = X
    print "MLP_INPUT shape = ", MLP_INPUT.get_shape()
    # Fully_Connected layer - 1
    num_outputs = 4000
    activation_fn = tf.nn.relu
    normalizer_fn = tf.contrib.layers.batch_norm
    dropout_param = 0.5
    y_f1 = fc_layer(MLP_INPUT , num_outputs, activation_fn, normalizer_fn, dropout_param, name='fc1')
    ...
    normalizer_fn = tf.contrib.layers.batch_norm
    dropout_param = 0.5
    y_f2 = fc_layer(y_f1, num_outputs, activation_fn, normalizer_fn, dropout_param, name='fc2')
    ...
    normalizer_fn = tf.contrib.layers.batch_norm
    dropout_param = 1.0
    y_f3 = fc_layer(y_f2, num_outputs, activation_fn, normalizer_fn, dropout_param, name='fc3')
