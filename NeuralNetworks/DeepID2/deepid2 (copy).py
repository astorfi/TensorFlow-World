...
# # Pairs and labels have been saved separately in HDF5 files.
# # The number of features and samples are extracted from data.
# X_train_L = np.transpose(TRAIN_FILE['main'],(0,3,2,1))
# X_train_R = np.transpose(TRAIN_FILE['aux'],(0,3,2,1))
# X_train = np.concatenate((X_train_L,X_train_R), axis=3)
# FileShape = X_train.shape
#
#
# # Create train HDF5
# hdf5_file_train = h5py.File('TRAIN' + '.hdf5', 'w')
# hdf5_file_train.create_dataset("pairs", shape=(FileShape[0], FileShape[1], FileShape[2], FileShape[3]),
#                          dtype=np.uint8)
# hdf5_file_train.create_dataset("labels", shape=(FileShape[0], 1),
#                          dtype=np.uint8)
# hdf5_file_train["labels"][:,:] = np.reshape(y_train,(y_train.shape[0],1))
# hdf5_file_train["pairs"][:,:,:,:] = X_train
#
#
#
#
# # TEST
# X_test_L = np.transpose(TEST_FILE['main'],(0,3,2,1))
# X_test_R = np.transpose(TEST_FILE['aux'],(0,3,2,1))
# X_test = np.concatenate((X_test_L,X_test_R), axis=3)
# FileShape = X_test.shape
#
# # Create test HDF5
# hdf5_file_test = h5py.File('TEST' + '.hdf5', 'w')
# hdf5_file_test.create_dataset("pairs", shape=(FileShape[0], FileShape[1], FileShape[2], FileShape[3]),
#                          dtype=np.uint8)
# hdf5_file_test.create_dataset("labels", shape=(FileShape[0], 1),
#                          dtype=np.uint8)
# hdf5_file_test["pairs"][:,:,:,:] = X_test
# hdf5_file_test["labels"][:,:] = np.reshape(y_test,(y_test.shape[0],1))
...
# Create train HDF5
hdf5_file_train = tables.open_file('TRAIN.hdf5', mode='w')
hdf5_file_test = tables.open_file('TEST.hdf5', mode='w')

X_train_hdf5 = hdf5_file_train.createEArray(hdf5_file_train.root, 'pairs',
                                      tables.UInt8Atom(),
                                      shape=(0, 224, 224, 3))
y_train = hdf5_file_train.createEArray(hdf5_file_train.root, 'labels',
                                       tables.UInt8Atom(),
                                      shape=(0, 1))

X_train_hdf5.append(X_train)
y_train_hdf5.append(y_train)

print X_train.root.pairs.shape
...
# Create test HDF5
X_test = hdf5_file_test.create_earray(hdf5_file_test.root, 'pairs', tables.Atom.uint8(), X_test)
y_test = hdf5_file_test.create_earray(hdf5_file_test.root, 'labels', tables.Atom.uint8(), np.reshape(y_test,(y_train.shape[0],1)))
...
X_train = TRAIN_FILE['pairs'][:N, :, :, :]
X_test = TEST_FILE['pairs'][:N, :, :, :]
y_train = TRAIN_FILE['labels'][:N, :]
y_test = TEST_FILE['labels'][:N, :]
