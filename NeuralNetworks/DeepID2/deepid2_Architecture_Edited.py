...
def Vector_to_Onehot_Fn(X,num_classes):
    V = np.zeros([X.shape[0],num_classes],dtype=np.float32)
    print V.shape
...
def get_total_loss(contrastive_loss, classification_loss_L, classification_loss_R):
    tf.add_to_collection('losses', 0.01 * contrastive_loss)
...
def get_batch_edited(start_idx, end_ind, input_frontal, input_profile, labels, label_frontal, label_profile):
    ...
    # print start_idx,end_ind
    # print "labels[start_idx:end_ind].shape",labels[start_idx:end_ind].shape
    # print "(len(range(start_idx, end_ind)), 1)",(len(range(start_idx, end_ind)), 1)
    y_cont = np.reshape(labels[start_idx:end_ind], (len(range(start_idx, end_ind)), 1))
    ...
    return pair_frontal_part_transpose, pair_profile_part_transpose, y_cont, y_frontal, y_profile
...
def MLP_Frontal(X, dropout_param=1):
...
def MLP_Profile(X, dropout_param=1):
    """This function create each branch os Siamese Architecture.
       Basically there are not two branches. They are the same!!
