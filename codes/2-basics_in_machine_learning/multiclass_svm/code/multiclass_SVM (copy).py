...
for i in range(FLAGS.num_steps):
    ...
    acc_train_temp = sess.run(accuracy, feed_dict={data_placeholder: batch_X,
                                                   label_placeholder: batch_y,
                                                   pred_placeholder: batch_X})

    batch_X_test, batch_y_test = next_batch(x_test, y_test, FLAGS.batch_size)
    acc_test_temp = sess.run(accuracy, feed_dict={data_placeholder: batch_X_test,
                                                  label_placeholder: batch_y_test,
                                                  pred_placeholder: batch_X_test})

    batch_accuracy.append(acc_test_temp)

    if (i + 1) % FLAGS.log_steps == 0:
        print('Step #' + str(i + 1))
        print('Loss = ' + str(temp_loss))
        print('accuracy train = ' + str(acc_train_temp))
        print('accuracy test = ' + str(acc_test_temp))
